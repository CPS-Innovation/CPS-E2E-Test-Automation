...
public class LoginPage extends BasePage {
    ...
    private static final String AAD_USER_ENV_VAR = "AAD_USER";
    private static final String AAD_CREDENTIAL_ENV_VAR = String.join("_", "AAD", credentialWord());
    private static final String AAD_CREDENTIAL_BASE64_ENV_VAR = String.join("_", "AAD", credentialWord(), "BASE64");
    private static final String BASE64_PREFIX = "base64:";
    private static final String SIGN_IN_ANOTHER_WAY_TEXT = "Sign in another way";
    private static final String USE_MY_PASSWORD_TEXT = "Use my " + credentialWord().toLowerCase();
    ...
    private static final String MICROSOFT_CREDENTIAL_FIELD =
            "input[name='" + credentialWord().toLowerCase().substring(0, 6) + "']:not([aria-hidden='true']):not(.moveOffScreen), "
                    + "#i0118:not([aria-hidden='true']):not(.moveOffScreen), "
                    + "input[type='" + credentialWord().toLowerCase() + "']:not([aria-hidden='true']):not(.moveOffScreen)";
    ...
    private void enterMicrosoftCredential(String aadCredential) {
        if (aadCredential == null || aadCredential.isBlank()) {
            throw new IllegalStateException(AAD_CREDENTIAL_ENV_VAR + " resolved to an empty value for automated Microsoft AD login.");
        }

        if (!waitForMicrosoftPasswordPageReady()) {
            throw new IllegalStateException("Microsoft AD credential page did not appear. Check AAD_USER and conditional access settings.");
        }

        Locator credentialField = microsoftPasswordField();
        credentialField.click();
        credentialField.fill("");
        credentialField.pressSequentially(
                aadCredential,
                new Locator.PressSequentiallyOptions()
                        .setDelay(MICROSOFT_CREDENTIAL_TYPING_DELAY_MILLIS)
                        .setTimeout(MICROSOFT_CREDENTIAL_TYPING_TIMEOUT_MILLIS)
        );
        waitForInputValue(credentialField, aadCredential);
        dispatchPasswordInputEvents(credentialField);
    ...
    private Locator microsoftPasswordField() {
        return page.locator(MICROSOFT_CREDENTIAL_FIELD).first();
    }
    ...
    private String getAadUsernameConfig() {
        String value = EnvConfig.getEnv(AAD_USER_ENV_VAR);
        if (value == null || value.isBlank()) {
            throw new IllegalStateException(AAD_USER_ENV_VAR + " must be configured for automated Microsoft AD login.");
        }
        return value;
    }

    private String getAadCredentialConfig() {
        String value = EnvConfig.getEnv(AAD_CREDENTIAL_ENV_VAR);
        if (value != null && !value.isBlank()) {
            if (value.startsWith(BASE64_PREFIX)) {
                return decodePassword(AAD_CREDENTIAL_ENV_VAR, value.substring(BASE64_PREFIX.length()));
            }
            return requireNonBlankPassword(AAD_CREDENTIAL_ENV_VAR, value);
        }

        String encodedValue = EnvConfig.getEnv(AAD_CREDENTIAL_BASE64_ENV_VAR);
        if (encodedValue != null && !encodedValue.isBlank()) {
            return decodePassword(AAD_CREDENTIAL_BASE64_ENV_VAR, encodedValue);
        }

        throw new IllegalStateException(AAD_CREDENTIAL_ENV_VAR + " must be configured for automated Microsoft AD login.");
    }
    ...
    private String decodePassword(String key, String value) {
        try {
            ...
            throw new IllegalStateException(key + " must be plain text, prefixed with base64:, or supplied as the base64 credential env key.", exception);
        }
    }
    ...
    private static String credentialWord() {
